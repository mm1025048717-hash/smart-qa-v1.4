...
export interface ParsedChunk {
  ...
  blocks: Array<{ type: 'text' | 'chart' | 'table' | 'kpi' | 'kpi-group' | 'gantt'; position: number; data: any }>;
}
...
export function parseRealtimeContent(content: string): ParsedChunk {
  ...
  // 收集所有内容块，按位置排序
  const allBlocks: Array<{ type: 'text' | 'chart' | 'table' | 'kpi' | 'kpi-group' | 'gantt'; position: number; data: any; text?: string }> = [];
  ...
  for (const marker of uniqueMarkers) {
    ...
    if (marker.start > lastEnd) {
      const textSegment = content.slice(lastEnd, marker.start).trim();
      if (textSegment) {
        finalBlocks.push({
          type: 'text',
          position: lastEnd,
