...
import { SKILL_LIBRARY, type SkillType } from './dataAgentSkills';
...
export class AgentOrchestrator {
  ...
  // 创建协作会话
  createSession(scenario: BusinessScenario, initialInputs?: Record<string, unknown>): CollaborationSession {
  ...
  private async executeTask(task: AgentTask): Promise<void> {
    ...
    try {
      ...
      this.sendMessage({
        fromAgentId: task.agentId,
        type: 'notify',
        content: `任务失败: ${task.task} - ${task.result.error}`,
        data: { taskId: task.id, error: task.result.error },
      });
    } finally {
