...
export function parseChartJson(jsonStr: string): any | null {
  ...
  try {
    ...
    if (chartData.type === 'line' || chartData.type === 'line-chart') {
...
export const ChartContainer = ({ 
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-xl border border-[#DEE0E3] p-5 shadow-sm"
    >
      {(title || showTabs) && (
      ...
      {activeTab === 'code' && (
        <div className="bg-[#F5F6F7] rounded-lg p-4 text-[12px] font-mono text-[#646A73] overflow-x-auto border border-[#DEE0E3]">
...
export const LineChartComponent = ({
  ...
  if (!data || !Array.isArray(data) || data.length === 0) {
    return (
      <ChartContainer title={title} delay={delay} summary={summary}>
    ...
    <ChartContainer title={title} delay={delay} summary={summary}>
      <ResponsiveContainer width="100%" height={200}>
        <RechartsBarChart 
          ...
          margin={{ top: 5, right: 5, left: horizontal ? 40 : -20, bottom: 0 }}
        >
  ...
  return (
    <ChartContainer title={title} delay={delay} summary={summary}>
      <div className="flex flex-col gap-4 md:flex-row md:items-center">
        ...
        <div className="flex-1 space-y-2 md:ml-4">
          {data.map((item, index) => (
...
export const FunnelChartComponent = ({ data, title, delay = 0, summary }: FunnelChartProps) => {
  return (
    <ChartContainer title={title} delay={delay} summary={summary}>
...
export const BoxPlotComponent = ({ data, title, delay = 0 }: BoxPlotProps) => {
  const range = data.max - data.min;
  const scale = (val: number) => ((val - data.min) / range) * 100;
  
  return (
    <ChartContainer title={title} delay={delay}>
...
export const MapChartComponent = ({ data, title, metric = '销售额', delay = 0 }: MapChartProps) => {
...
export const QuadrantChartComponent = ({ data, xKey, yKey, title, quadrants, delay = 0 }: QuadrantChartProps) => {
  return (
    <ChartContainer title={title} delay={delay}>
      <div className="relative">
        ...
        <ScatterChartComponent data={data} xKey={xKey} yKey={yKey} delay={delay} />
      </div>
    </ChartContainer>
  );
};
...
export const SmartChart = ({ chartData, delay = 0, onDrillDown }: SmartChartProps) => {
  ...
  switch (type) {
    ...
    case 'line-chart':
      if (finalProps.type === 'year-comparison') {
        return <YearComparisonChart {...finalProps} delay={delay} />;
      }
      // 验证折线图必需的参数
      ...
      return <LineChartComponent {...finalProps} delay={delay} />;
    ...
    case 'bar-chart':
      ...
      return <BarChartComponent {...finalProps} delay={delay} />;
    ...
    case 'pie-chart':
      ...
      return <PieChartComponent {...restProps} delay={delay} />;
    ...
    case 'scatter-chart':
      ...
      return <ScatterChartComponent {...restProps} delay={delay} />;
    ...
    case 'funnel-chart':
      ...
      return <FunnelChartComponent {...restProps} delay={delay} />;
    ...
    case 'box-plot':
      ...
      return <BoxPlotComponent {...restProps} delay={delay} />;
    ...
    case 'map-chart':
      ...
      return <MapChartComponent {...restProps} delay={delay} />;
    ...
    case 'quadrant-chart':
      ...
      return <QuadrantChartComponent {...restProps} delay={delay} />;
    
    case 'year-comparison':
      return <YearComparisonChart {...restProps} delay={delay} />;
    ...
    case 'area-chart':
      ...
      if (!finalProps.xKey || !finalProps.yKeys || !Array.isArray(finalProps.yKeys) || finalProps.yKeys.length === 0) {
        return (
          <div className="my-4 p-4 bg-[#FFF5F5] border border-[#FFE5E5] rounded-xl">
            <p className="text-[13px] text-[#FF3B30]">面积图配置错误：缺少 xKey 或 yKeys</p>
            <p className="text-[11px] text-[#86868b] mt-1">数据字段: {restProps.data?.[0] ? Object.keys(restProps.data[0]).join(', ') : '无'}</p>
          </div>
        );
      }
      return <LineChartComponent {...finalProps} showArea={true} delay={delay} />;
    
    default:
      // 通用图表渲染器：尝试自动适配未知类型
      return <UniversalChartRenderer chartData={finalProps} chartType={type} delay={delay} />;
  }
};

// 通用图表渲染器 - 自动适配任何图表类型
const UniversalChartRenderer = ({ chartData, chartType, delay = 0 }: { chartData: any; chartType: string; delay?: number }) => {
  const { data, xKey, yKey, yKeys, title, ...restProps } = chartData;
  ...
  // 最后的备选方案：显示数据表格
  return (
    <ChartContainer title={title || chartType} delay={delay}>
      <div className="py-4">
